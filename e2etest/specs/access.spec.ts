...
test.describe('Access control', () => {
  ...
  test('should delete a group with confirmation @regression', async ({
    ...
    await page.locator('.access-control__form button[type="submit"]').click()
    await page.waitForSelector('.access-control__group-item')
    await page.locator('.access-control__group-delete').first().click()
    await expect(page.locator('.access-control__confirm-overlay')).toBeVisible()
    await page.locator('.access-control__confirm-overlay .access-control__confirm-dialog button').first().click()
    await expect(page.locator('.access-control__group-item')).toHaveCount(0, { timeout: 5000 })
  })
})
