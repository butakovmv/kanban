...
class GrantPermissionOperationImplTest {
    ...
    private val operation =
        GrantPermissionOperationImpl(groupRepository, permissionRepository, groupPermissionRepository)
    ...
    @Test
    fun `should grant permission when group and permission exist`() =
        runTest {
            ...
            coEvery { groupPermissionRepository.grant("group-1", "perm-1") } returns Unit

            val result =
                operation.execute(
                    GrantPermissionOperation.Arg(groupId = "group-1", permissionId = "perm-1"),
                )

            assertIs<GrantPermissionOperation.Result.Success>(result)
            coVerify { groupRepository.findById("group-1") }
            coVerify { permissionRepository.findById("perm-1") }
            coVerify { groupPermissionRepository.grant("group-1", "perm-1") }
        }
    ...
    fun `should fail when group not found`() =
        runTest {
            ...
            val result =
                operation.execute(
                    GrantPermissionOperation.Arg(groupId = "missing", permissionId = "perm-1"),
                )

            val failure = assertIs<GrantPermissionOperation.Result.Failure>(result)
            ...
            coVerify(inverse = true) { groupPermissionRepository.grant(any(), any()) }
        }
    ...
    fun `should fail when permission not found`() =
        runTest {
            ...
            val result =
                operation.execute(
                    GrantPermissionOperation.Arg(groupId = "group-1", permissionId = "missing"),
                )
